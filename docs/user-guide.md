# iOS 元書輸入法 - 蝦米方案使用指南

---

## 一、安裝方式

### 安裝需求

- **iOS/iPadOS**: 元書輸入法 App

  ![01.元書icon](images/01-1元書icon.png)

- 支援 iOS 16 或以上版本

- 下載位置：https://apps.apple.com/us/app/%E5%85%83%E4%B9%A6%E8%BE%93%E5%85%A5%E6%B3%95/id6744464701?l=zh-Hant-TW

### 啟用輸入法安裝完成後，需要在系統設定中啟用元書輸入法：

1. 開啟「設定」App

   <img src="images/01-2設定icon.png" alt="01-2設定icon" />

2. 選擇「一般」→「鍵盤」→「鍵盤」

   <img src="images/01-3一般.PNG" alt="01-3一般" />

   

   <img src="images/01-4鍵盤.PNG" alt="01-4鍵盤" />

   

   <img src="images/01-5鍵盤.PNG" alt="01-5鍵盤" />

   

3. 點擊「新增鍵盤」

   <img src="images/01-6新增鍵盤.PNG" alt="01-6新增鍵盤" />

   

4. 在第三方鍵盤中選擇「元書輸入法」

   <img src="images/01-7元書輸入法.PNG" alt="01-7元書輸入法" />

   

5. 開啟「允許完整取用」

   <img src="images/01-8元書輸入法.PNG" alt="01-8元書輸入法" />

   

   <img src="images/01-9允許完整取用.PNG" alt="01-9允許完整取用" />

   

6. 若長按「地球」後，選單中沒有「元書輸入法」，請重新開機。

   <img src="images/01-10地球選單.png" alt="01-10地球選單" />

   

7. 選擇「一般」→「鍵盤」→「鍵盤」→「聽寫語言」

   <img src="images/01-11聽寫語言.PNG" alt="01-11聽寫語言" />

   

8. 取消勾選「國語 - 中國大陸」

   <img src="images/01-12國語中國大陸.PNG" alt="01-12國語中國大陸" />

   

### 方案配置說明本方案提供多種配置選擇，滿足不同使用需求：

#### 基本配置

- **中文輸入**：純中文輸入，不含英文詞庫，可中英混輸
- **中文輸入+英文詞庫**：包含英文詞庫，支援英文自動補全，可中英混輸
- **中文輸入+屏蔽無效鍵**：純中文輸入 + 屏蔽無效按鍵，不可中英混輸
- **中文輸入+英文詞庫+屏蔽無效鍵**：完整功能 + 屏蔽無效按鍵，英文字母多於5字可中英混輸

 **★補充說明★**：方案中所稱之「屏蔽無效鍵」，是讓你無法按出無效鍵，如按完ab，無法按c，但視覺上無法讓c鍵變淡，這是元書輸入法沒有支援的功能。

#### 皮膚主題

- **蝦米輸入法皮膚設計器** ：https://ryanwuson.github.io/rime-liur-ios-skin/

- 使用者可以使用習慣調整：

  - **按鍵佈局**：包含「空白鍵長度」、「左、右手持機」、「上下滑功能」及「長按功能」

    <img src="images/Skin01.png" alt="Skin01" class="wide-img" />

  - **工具配置**：包含25項實用工具，使用者可依需求於工具列放置10項

    <img src="images/Skin02.png" alt="Skin02" class="wide-img" />

  - **基礎外觀**：調整亮/暗色鍵盤顏色

    <img src="images/Skin01.png" alt="Skin01" class="wide-img" />
    <img src="images/Skin02.png" alt="Skin02" class="wide-img" />
    <img src="images/Skin03.png" alt="Skin03" class="wide-img" />

  - **進階微調**：細緻調整「26鍵」、「數字」、「符號」及「Emoji」鍵盤。

    <img src="images/Skin04.png" alt="Skin04" class="wide-img" />

- 編輯皮膚後，請依序：

  - 按右上角 **i 圖示**，編輯皮膚名稱及作者資訊

  - 按右上角 **導出配置 (.cskin)**，即可導出自定皮膚，包含亮暗色26鍵及 **自動截圖**

    <img src="images/Skin05.png" alt="Skin05" class="wide-img" />

    <img src="images/Skin05-2.PNG" alt="Skin05-2" class="wide-img" />

  - 透過 iOS 元書輸入法的長按鍵盤皮膚並 **「運行 main.jsonnet」** 即可使用

    <img src="images/Skin05-1.png" alt="Skin05-1" class="wide-img" />

- 皮膚分享及再次修改：

  - 導出之.cskin皮膚可分享給他人

  - 若需修改自己或他人之皮膚，可按右上 **匯入配置 (.cskin)**，進行修改。

    <img src="images/Skin06.png" alt="Skin06" class="wide-img" />

    <img src="images/Skin06-1.png" alt="Skin06-1" class="wide-img" />

- **簡易操作說明**：https://github.com/ryanwuson/rime-liur-ios-skin

  

### 手機直接安裝之步驟

1. **選擇適合方案並複製網址**
   
   - **中文輸入**：純中文輸入，不含英文詞庫
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/main
   
   - **中文輸入+英文詞庫**：包含英文詞庫，支援英文自動補全
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/english
   
   - **中文輸入+屏蔽無效鍵**：純中文輸入 + 屏蔽無效按鍵
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/main-block
   
   - **中文輸入+英文詞庫+屏蔽無效鍵**：完整功能 + 屏蔽無效按鍵
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/english-block

2. **匯入元書輸入法**
   
   - 開啟元書輸入法 App
   
     ![01.元書icon](images/01-1元書icon.png)
   
   - 選擇「輸入方案」
   
     <img src="images/01-13輸入方案.PNG" alt="01-13輸入方案" />
   
     
   
   - 點擊「下載方案」
   
     <img src="images/01-14下載方案.PNG" alt="01-14下載方案" />
   
     
   
     <img src="images/01-15下載方案.PNG" alt="01-15下載方案" />
   
     
   
   - 點擊「㊉」
   
     <img src="images/01-16㊉.PNG" alt="01-16㊉" />
   
     
   
   - 輸入「方案名稱(自訂)」、貼上「下載網址」並儲存
   
     <img src="images/01-17下載網址.PNG" alt="01-17下載網址" />
   
     
   
   - 點擊右方綠色按鍵下載，等待下載完成
   
     <img src="images/01-18下載完成.PNG" alt="01-18下載完成" />
   
     
   
     <img src="images/01-19下載完成.PNG" alt="01-19下載完成" />
   
     
   
     <img src="images/01-20下載完成.PNG" alt="01-20下載完成" />
   
     
   
   - 點擊「方案目錄切換」
   
     <img src="images/01-14下載方案.PNG" alt="01-14下載方案" />
   
     
   
     <img src="images/01-21方案目錄切換.PNG" alt="01-21方案目錄切換" />
   
     
   
   - 依序選擇「RimeUserData」
   
     <img src="images/01-22RimeUserData.PNG" alt="01-22RimeUserData" />
   
     
   
   - 選擇「方案名稱(自訂)」
   
     <img src="images/01-23方案名稱.PNG" alt="01-23方案名稱" />
   
     
   
   - 選擇「rime-liur-ios」
   
     <img src="images/01-24rime-liur-ios.PNG" alt="01-24rime-liur-ios" />
   
     
   
   - 點擊「打開」
   
     <img src="images/01-25打開.PNG" alt="01-25打開" />
   
     
   
     <img src="images/01-26清單.PNG" alt="01-26清單" />
   
     
   
3. **安裝皮膚主題**
   
   - **從皮膚設計器中直接匯出：**
   
     - 打開 iOS 瀏覽器（以 Safari 為例），訪問 **蝦米輸入法皮膚設計器**
   
       https://ryanwuson.github.io/rime-liur-ios-skin/
   
     - 按右上角 **導出配置 (.cskin)**，即會出現下載視窗
   
     <img src="images/01-27下載視窗.PNG" alt="01-27下載視窗" />
   
     
   
     - 將皮膚儲存至「檔案.app」
   
   
     <img src="images/01-28下載項目.png" alt="01-28下載項目" />
   
     
   
     <img src="images/01-29下載項目.PNG" alt="01-29下載項目" />
   
     
   
     <img src="images/01-30下載項目.PNG" alt="01-30下載項目" />
   
     
   
     <img src="images/01-31下載項目.PNG" alt="01-31下載項目" />
   
     
   
     - 點擊.cskin檔案，以元書輸入法 App 開啟
   
   
     <img src="images/01-32下載項目.PNG" alt="01-32下載項目" />
   
     
   
     - 長按「皮膚主題」，點擊 **「運行 main.jsonnet」**，即可使用
   
   
     <img src="images/01-33勾選皮膚.PNG" alt="01-33勾選皮膚" />

### 透過電腦安裝之步驟

1. **選擇適合方案，點擊網址下載至電腦並解壓縮**

   - **中文輸入**：純中文輸入，不含英文詞庫
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/main

   - **中文輸入+英文詞庫**：包含英文詞庫，支援英文自動補全
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/english

   - **中文輸入+屏蔽無效鍵**：純中文輸入 + 屏蔽無效按鍵
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/main-block

   - **中文輸入+英文詞庫+屏蔽無效鍵**：完整功能 + 屏蔽無效按鍵
     https://codeload.github.com/ryanwuson/rime-liur-ios/zip/refs/heads/english-block

2. **將電腦及iPhone（或iPad）連線到同一 Wi-Fi**

   - 開啟元書輸入法

     ![01.元書icon](images/01-1元書icon.png)

   - Wi-Fi檔案傳輸

     <img src="images/01-34wifi.PNG" alt="01-34wifi" />

   - 記下網址（每個人網址不同），將手機維持開啟，並停留在此頁面

     <img src="images/01-35網址.PNG" alt="01-35網址" />

   - 打開電腦瀏覽器，貼上並前往前一步顯示之網址，雙擊 **RimeUserData** 資料夾

     <img src="images/01-36瀏覽器.png" alt="01-36瀏覽器" class="wide-img" />

   - 點擊右上角的「↑」圖示

     <img src="images/01-37RimeUserData.png" alt="01-37RimeUserData" class="wide-img" />

   - 上傳解壓縮後之資料夾

     <img src="images/01-37upfolder.png" alt="01-37upfolder" class="wide-img" />

   - 資料夾結構如下

     <img src="images/01-38RimeUserData.png" alt="01-38RimeUserData" class="wide-img" />

     <img src="images/01-39RimeUserData.png" alt="01-39RimeUserData" class="wide-img" />

   - 回到 iPhone 或 iPad，選擇「輸入方案」

     <img src="images/01-13輸入方案.PNG" alt="01-13輸入方案" />

   - 點擊「方案目錄切換」

     <img src="images/01-14下載方案.PNG" alt="01-14下載方案" />

     <img src="images/01-21方案目錄切換.PNG" alt="01-21方案目錄切換" />

     

   - 依序選擇「RimeUserData」

     <img src="images/01-22RimeUserData.PNG" alt="01-22RimeUserData" />

     

     - 選擇「方案名稱」

       <img src="images/01-23方案名稱.PNG" alt="01-23方案名稱" />

       

     - 選擇「rime-liur-ios」

       <img src="images/01-24rime-liur-ios.PNG" alt="01-24rime-liur-ios" />

       

     - 點擊「打開」

       <img src="images/01-25打開.PNG" alt="01-25打開" />

       

       <img src="images/01-26清單.PNG" alt="01-26清單" />

       

   - **從皮膚設計器中直接匯出**

     打開電腦瀏覽器（以 Safari 為例），訪問 **蝦米輸入法皮膚設計器**

     https://ryanwuson.github.io/rime-liur-ios-skin/

     按右上角 **導出配置 (.cskin)**，即會出現下載主題

     **下載後，將 .cskin 副檔名變更為 .zip，並解壓縮**

   - 透過前述 iPhone（iPad）與電腦於 **同一 WI-FI 網段** 之連線，雙擊進入Skins資料夾

     <img src="images/01-40瀏覽器.png" alt="01-40瀏覽器" class="wide-img" />

   - 點擊右上角的「↑」圖示

     <img src="images/01-41skin.png" alt="01-41skin" class="wide-img" />

   - 上傳解壓縮後之資料夾

     <img src="images/01-37upfolder.png" alt="01-37upfolder" class="wide-img" />

   - 資料夾結構如下

     <img src="images/01-42skin.png" alt="01-42skin" class="wide-img" />

   - 回到 iPhone 或 iPad，選擇「鍵盤皮膚」

     <img src="images/01-43鍵盤皮膚.PNG" alt="01-43鍵盤皮膚" />

   - 長按「皮膚主題」，點擊 **「運行 main.jsonnet」**，即可使用

     <img src="images/01-33勾選皮膚.PNG" alt="01-33勾選皮膚" />

### 注意事項若元書輸入法有更新，導致無法打字，請重新部署

<img src="images/重新部署.png" alt="重新部署" />



---

## 二、元書輸入法基本設定

### 基本設定項目

#### 基本設定

- **位置**：鍵盤設定

    <img src="images/02-1鍵盤設定.PNG" alt="02-1鍵盤設定" />

  

    <img src="images/02-2設定.png" alt="02-2設定" />

#### 簡繁轉換設定

- **位置**：RIME

    <img src="images/02-3RIME.png" alt="02-3RIME" />

  

- **簡繁切換**→`simplification`

    <img src="images/02-4簡繁切換.png" alt="02-4簡繁切換" />

  

#### 候選Comment設定
- **說明**：開啟後，「vrsf 提示」、「讀音查詢」、「快打模式」等功能才會有提示字

- **位置**：鍵盤設定

    <img src="images/02-1鍵盤設定.PNG" alt="02-1鍵盤設定" />

  

- **候選Comment**：開啟

    <img src="images/02-5候選Comment.png" alt="02-5候選Comment" />

  

    <img src="images/02-6候選Comment效果.png" alt="02-6候選Comment效果" />

---

## 三、鍵盤介面與操作

### 鍵盤佈局

#### 主要區域
- **候選字區**：顯示輸入候選字

- **工具列**：快速功能按鍵

- **功能鍵區**：Shift、123、空格、Enter、Backspace

- **字母鍵區**：QWERTYUIOP、ASDFGHJKL、ZXCVBNM

  ![03-0鍵盤](images/03-0鍵盤.png)

### 基本操作手勢

#### 點擊操作
- **字母鍵**：輸入蝦米編碼對應之字母

- **空格鍵**：選擇第1個候選字

- **候選字**：直接點選候選字上屏

    <img src="images/03-3點擊操作.png" alt="03-3點擊操作" />

  

  

#### 長按操作
- **字母鍵**：顯示該鍵的符號選單或變體選項
    <img src="images/03-3-2長按操作字母.gif" alt="03-3-2長按操作字母" />

- **空格鍵**：開啟方案選擇器，可在「蝦米輸入法」及「Easy English」間切換
    <img src="images/03-3-3長按操作空白.gif" alt="03-3-3長按操作空白" />

  

- **123鍵**：顯示數學符號選單（+、-、×、÷、=、≠）

    <img src="images/03-3-4長按操作123.gif" alt="03-3-4長按操作123" />

  

- **「,」及「.」鍵**：開啟「日期時間」選單

    <img src="images/03-3-5長按操作，。.gif" alt="03-3-5長按操作，。" />

#### 滑動操作

##### 一般上滑及下滑功能

![03-11一般上下滑](images/03-11一般上下滑.png)

- **Q～P鍵下滑**：快速輸入 **數字1～0**，若「候選字區」有候選字，也可透過 **Q～P鍵下滑** 來選字

    <img src="images/03-11-1QP鍵下滑.gif" alt="03-11-1QP鍵下滑" />

  

- **Q～P鍵上滑、A～L鍵上下滑、X～B鍵上滑**：輸出該鍵左上或右上之符號

    <img src="images/03-11-2QP鍵上滑.gif" alt="03-11-2QP鍵上滑" />

  

- **空格鍵上滑**：切換擴充字集

    <img src="images/03-11-3空白鍵上滑.gif" alt="03-11-3空白鍵上滑" />

  

- **Shift鍵上滑**：切換中文及純英文鍵盤

    <img src="images/03-11-4Shift鍵上滑.gif" alt="03-11-4Shift鍵上滑" />

  

- **Shift鍵下滑**：切換至下一個輸入法 (解決 SE 機型無地球問題)

    <img src="images/03-11-4Shift鍵下滑.gif" alt="03-11-4Shift鍵下滑" />
    
    
    
- **「Enter」鍵下滑**：換行，部分 App 無換行功能，下滑 Enter 可強制換行

    <img src="images/03-11-5「Enter」鍵下滑.gif" alt="03-11-5「Enter」鍵下滑" />
    
    
    
- **「123」鍵上滑**：切換為符號鍵盤

    <img src="images/03-11-5「123」鍵上滑.gif" alt="03-11-5「123」鍵上滑" />

  

##### 特殊上滑及下滑功能

![03-11特殊上下滑](images/03-11特殊上下滑.png)

- **A鍵上滑**：上滑A鍵輸出之<code>&#96;</code>，可開啟「字母變體」及「數字變體」功能

    <img src="images/03-11-6A鍵上滑.gif" alt="03-11-6A鍵上滑" />

  

- **G鍵上滑**：上滑G鍵輸出之右括號`]`，可使英文字母之字首大寫

- **G鍵上滑2次**：上滑G鍵2次輸出之右括號`]]`，可使每個英文字母均大寫

    <img src="images/03-11-7G鍵上滑.gif" alt="03-11-7G鍵上滑" />

  

- **K鍵上滑**：上滑K鍵輸出之分號 `;`，可開啟「臨時造詞」功能（註：不適用於屏蔽無效字方案）

    <img src="images/03-11-8K鍵上滑.gif" alt="03-11-8K鍵上滑" />

  

- **K鍵上滑2次**：上滑K鍵2次輸出之分號 `;;`，可開啟「讀音查詢」功能（註：需開啟候選Comment）

    <img src="images/03-11-9K鍵上滑2.gif" alt="03-11-9K鍵上滑2" />

  

- **K鍵及L鍵依序上滑**：上滑K鍵輸出之分號 `;`，上滑L鍵輸出之單引號 `'` ，組合成`;'`，可開啟「拼音輸入」功能

    <img src="images/03-11-10KL鍵上滑.gif" alt="03-11-10KL鍵上滑" />

  

- **L鍵上滑**：輸入蝦米編碼 + 上滑L鍵輸出之單引號 `'`，可開啟「同音字」功能

    <img src="images/03-11-11L鍵上滑.gif" alt="03-11-11L鍵上滑" />

  

- **Z～M鍵下滑**：分別為 **Z** 「句首」、**M** 「句尾」、**N** 「全選」、**X** 「剪下」、**V** 「貼上」、**C** 「複製」及 **B** 「Tab」功能

    <img src="images/03-11-12ZM鍵下滑.gif" alt="03-11-12ZM鍵下滑" />

  

- **Z、N及M鍵上滑**：分別為 **Z** 「單手模式」、**N** 「選擇第2候選字」及 **M** 「選擇第3候選字」功能

    <img src="images/03-11-13ZNM鍵上滑.gif" alt="03-11-13ZNM鍵上滑" />

  

##### Backspace鍵之滑動功能

![03-11Delete上下滑](images/03-11Delete上下滑.png)

- **輸入蝦碼字碼後左滑**：清除所有字碼，方便刪除錯誤之字碼

    <img src="images/03-11-14Delete01.gif" alt="03-11-14Delete01" />

  

- **輸入臨時造詞之字碼後下滑**：清除臨時造詞

    <img src="images/03-11-14Delete02.gif" alt="03-11-14Delete02" />

  

- **對已上屏文字之上滑**：清除所有上屏之文字

- **清除上屏文字後下滑**：回復前次清除之上屏文字

    <img src="images/03-11-14Delete03.gif" alt="03-11-14Delete03" />

  

- **對已上屏文字長按並左滑及右滑**：長按並往左滑動可逐字刪除已上屏之文字，保持長按並往右滑動，則可逐字回復刪除之文字

    <img src="images/03-11-14Delete05.gif" alt="03-11-14Delete05" />

  

##### 全鍵盤（除了 Backspace 鍵）左右滑動：移動編輯狀態時之游標

![03-12全鍵盤左右滑動](images/03-12全鍵盤左右滑動.png)

<img src="images/03-11-15全鍵盤.gif" alt="03-11-15全鍵盤" />

### 工具列功能

#### 工具按鍵

![03-13工具列](images/03-13工具列.png)

1. **設定面版**：關於元書輸入法App之相關設定

   ![03-14設定面版](images/03-14設定面版.jpg)

2. **純英文鍵盤**：切換為不含英文字庫之純英文鍵盤

   ![03-15純英文鍵盤](images/03-15純英文鍵盤.jpg)

3. **簡繁轉換**：快速切換及輸入繁體中文及簡體中文

   ![03-16簡繁轉換](images/03-16簡繁轉換.jpg)

4. **符號鍵盤**：切換至50個分類的符號鍵盤

   ![03-17符號鍵盤](images/03-17符號鍵盤.jpg)

5. **Emoji鍵盤**：切換至Emoji鍵盤

   ![03-18Emoji鍵盤](images/03-18Emoji鍵盤.jpg)

6. **鍵盤收折**：隱藏鍵盤

   

### 候選字操作

#### 選字方式
1. **空格選字**：按空格鍵選擇第一個候選字

2. **點擊選字**：直接點擊想要的候選字

3. **數字選字**：下滑Q～P鍵（對應1～9數字鍵）選擇對應候選字

4. **按右側展開鍵**：顯示更多候選字

   <img src="images/03-19數字選字.gif" alt="03-19數字選字" />

---

## 四、基本輸入

### 蝦米字碼輸入

- 直接輸入蝦米字碼，按 `Space` 出字，或下滑 Q ～ P 鍵，可以 1 ～ 9 數字鍵選字
- 支援標準字碼、簡碼輸入
- 具備 VRSF 選字功能

### 中文及英文詞庫混合輸入在完整版（中打含英文詞庫版）中，英文單詞會自動出現在候選選單中：

- **自動補全**：輸入 `app` 會在候選中顯示 `apple`、`application` 等
- **空白鍵上屏**：選擇英文單詞後按 `Space` 即可上屏
- **大小寫轉換**：
  - 輸入英文單詞後按 `]` → 首字母大寫（如：`word` → `Word`）
  - 輸入英文單詞後按 `]]` → 全部大寫（如：`word` → `WORD`）

### 英文詞庫輸入模式

- **快捷鍵**：長按`space`鍵切換
- **說明**：切換至 Easy English 模式，內建英文詞組，輸入完成按 `Space` 上屏

### 純英文輸入模式

- **快捷鍵**：`shift`鍵上滑，或是直接點按工具列的「英」切換
- **說明**：切換至英文輸入模式，打字直出英文，不帶英文詞庫

---

## 五、造詞功能

### 臨時造詞臨時造詞適合日常使用，造詞後可立即使用，系統會自動儲存。

#### 造詞操作方式

1. `上滑K鍵`輸入 `;` 進入造詞模式（顯示《造詞》▸）
2. 依序輸入每個字的編碼，`上滑A鍵`用 `` ` `` 分隔
3. 按空白鍵確認上屏

#### 刪除臨時造詞打出臨時造詞之編碼後，`下滑Backspace鍵滑即可刪除該詞條

#### 使用範例

- **造「臺北車站」**：``;te`ffl`cj`lzo`` → 臺北車站
- **造「行政院」**：``;mv`ezp`br`` → 行政院

#### 臨時造詞編碼規則造詞後會自動產生 4 碼的快捷編碼，取每個字的「首碼」組合：

| 詞長 | 編碼規則 | 說明 |
|:----:|:--------:|:-----|
| 2 字詞 | ABBB | 第1字首碼 + 第2字首碼×3 |
| 3 字詞 | ABCC | 第1字首碼 + 第2字首碼 + 第3字首碼×2 |
| 4字以上 | ABCZ | 第1字首碼 + 第2字首碼 + 第3字首碼 + 末字首碼 |

#### 編碼範例

| 詞彙 | 各字首碼 | 造詞編碼 |
|:----:|:---------|:--------:|
| 台北 | 台(u) 北(f) | `ufff` |
| 行政院 | 行(m) 政(e) 院(b) | `mebb` |
| 台北市政府 | 台(u) 北(f) 市(l) 政(e) 府(l) | `ufll` |

#### 注意事項

- 造詞後可立即使用，不需重新部署
- 關閉輸入法時會自動保存

### 直接編輯造詞如需批量新增詞彙或與他人分享自訂詞庫，可直接編輯詞庫檔案。

#### 操作步驟

1. 開啟「元書輸入法」→「文件管理」→「RimeUserDate」→「方案文件夾」
2. 找到自訂詞庫檔案 `openxiami_CustomWord.dict.yaml`
3. 在檔案中新增詞條（格式：`詞彙<Tab>編碼`），不受限於臨時造詞編碼規則
4. 儲存檔案
5. 重新部署
6. 也可透過 iMazing 之類的傳檔程式，直接將電腦端的自訂詞庫檔案 `openxiami_CustomWord.dict.yaml`放入相對應的資料夾

---

## 六、同音選字

### 功能說明

- **操作**：輸入候選字後點擊 **左下角的「同音」鍵** 或上滑L鍵輸出 `'`

- **說明**：顯示該字的同音字及其字碼，方便查找同音異字

- **注意**：簡體模式下亦可對應相關同音字，如「发」的同音字有「伐」、「瞂」及「法」。

    <img src="images/06-1同音選字.gif" alt="06-1同音選字" />

### 應用場景

- 忘記某個同音字的編碼時，可透過已知的字查找
- 快速輸入同音異字，不需重新輸入編碼

---

## 七、拼音輸入

### 拼音輸入

- **引導鍵**：上滑K鍵 + 上滑L鍵輸出 `;'`
- **提示**：《拼音輸入》▸
- **說明**：使用漢語拼音輸入中文，支援數字1～5（下滑QWERT）鍵入聲調，可連續輸入
- **舉例**：`;'binfenye2danwan2taibei` → 繽紛耶誕玩台北

---

## 八、字母變體

### 字母變體

- **引導鍵**：上滑A鍵輸出<code>&#96;</code>
- **輸入**：<code>&#96;a</code> ～ <code>&#96;z</code>
- **提示**：<code>&#96;</code>《變體a》
- **說明**：輸入圈圈字母、方框字母、雙線字母、哥德體等變體
- **舉例**：<code>&#96;a</code> → Ⓐ ⓐ 🅐 🄰 🅰 ...

---

## 九、變體英數

### 變體Aa（首字母大寫）

- **引導鍵**：上滑A鍵 + 上滑J鍵輸出 <code>&#96;/</code>
- **提示**：《變體Aa》
- **說明**：用 `.` 分隔單字，單字首字母大寫，可連續輸入
- **舉例**：<code>&#96;/hello.world</code> → Ⓗⓔⓛⓛⓞ Ⓦⓞⓡⓛⓓ

### 變體aa（全小寫）

- **引導鍵**：上滑A鍵 + 上滑J鍵2次輸出 <code>&#96;//</code>
- **提示**：《變體aa》
- **說明**：用 `.` 分隔單字，單字均為小寫，可連續輸入
- **舉例**：<code>&#96;//hello.world</code> → ⓗⓔⓛⓛⓞ ⓦⓞⓡⓛⓓ

### 變體AA（全大寫）

- **引導鍵**：上滑A鍵 + 上滑J鍵3次輸出 <code>&#96;///</code>
- **提示**：《變體AA》
- **說明**：用 `.` 分隔單字，單字均為大寫，可連續輸入
- **舉例**：<code>&#96;///hello.word</code> → ⒽⒺⓁⓁⓄ ⓌⓄⓇⓁⒹ

### 變體數字 - 連打

- **引導鍵**：上滑A鍵 + 上滑J鍵 + 上滑L鍵輸出 <code>&#96;/'</code>
- **提示**：《變體數字》
- **說明**：用 `.` 分隔數字，如果需要用數字鍵選字，於結尾按 `'` 即可
- **舉例**：<code>&#96;/'123</code> → ①②③

### 變體數字 - 單選

- **引導鍵**：上滑A鍵 + 上滑L鍵輸出 <code>&#96;'</code>
- **提示**：<code>&#96;'</code>《變體數字》
- **範圍**：<code>&#96;'00</code> ～ <code>&#96;'50</code>
- **舉例**：<code>&#96;'01</code> → ① ⓵ ⑴ ⒈ 🄂 １ ₁ ¹ 𝟏 𝟙 𝟷 𝟣 𝟭 Ⅰ ⅰ ㊀ ㈠ ...

---

## 十、快打模式 (需開啟候選Comment)

### 功能說明

- **開關**：`,,sp`（輸入後按空格）
- **狀態**：正常 ↔ 快打
- **說明**：開啟後會提示可用的簡碼，幫助使用者更快速輸入

### 功能特色

1. **簡碼提示**：當輸入的字有更短的編碼時，會在候選字旁顯示簡碼提示
2. **提升效率**：讓使用者學習並記住常用字的簡碼，加快打字速度
3. **即時顯示**：輸入時即時顯示可用的簡碼選項

### 提示格式

- `簡碼▸⟨x⟩`：表示該字可用字碼 `x` 輸入
- 簡碼提示會顯示在候選字的註解區域

### 使用範例

| 輸入 | 候選字 | 簡碼提示 |
|:----:|:------:|:---------|
| `ttmb` | 頂 | ▸簡碼⟨TTA⟩ |
| `lonm` | 毫 | ▸簡碼⟨LVM⟩⟨LNM⟩ |

### 注意事項

- 快打模式主要用於學習簡碼

- 熟悉簡碼後可關閉此模式以減少視覺干擾

- 若需常駐此功能：

  1. 至方案中開啟 liur.schema.yaml

  2. 尋找

     name: quick_mode
     reset: 0
     states: [ 正常, 快打提示 ]

  3. 將 reset 改為 1

  4. 儲存並重新部署

---

## 十一、強制快打模式 (需開啟候選Comment)

### 功能說明

- **開關**：`,,sf`（輸入後按空格）
- **狀態**：正常 ↔ 強制快打
- **說明**：開啟後會提示可用的簡碼。使用者如果不是用最簡碼，按 space 鍵無法上屏

### 功能特色

1. **簡碼提示**：當輸入的字有更短的編碼時，會在候選字旁顯示簡碼提示
2. **提升效率**：強制使用者學習並記住常用字的簡碼
3. **即時顯示**：輸入時即時顯示可用的簡碼選項

### 提示格式

- `簡碼▸⟨x⟩`：表示該字可用字碼 `x` 輸入
- 簡碼提示會顯示在候選字的註解區域

### 使用範例

|  輸入  | 候選字 | 簡碼提示        |
| :----: | :----: | :-------------- |
| `ttmb` |   頂   | ▸簡碼⟨TTA⟩      |
| `lonm` |   毫   | ▸簡碼⟨LVM⟩⟨LNM⟩ |

### 注意事項

- 快打模式主要用於學習簡碼

- 熟悉簡碼後可關閉此模式以減少視覺干擾

- 若需常駐此功能：
  1. 至方案中開啟 liur.schema.yaml

  2. 尋找

      name: force_quick_mode
      reset: 0
      states: [ 正常, 強制快打 ]

  3. 將 reset 改為 1

  4. 儲存並重新部署

- 1. 儲存並重新部署

---

## 十二、聯想字

### 功能特色

- 預設開啟聯想字功能，**簡體字也有相對應的聯想字**

- 可依上屏的第1個字進行聯想

- 1次提供24個候選聯想字，自工具列下滑即可展開

  <img src="images/07聯想字.gif" alt="07聯想字" />

### 注意事項

- 因 RIEM 候選字模組問題，無法判斷前後語意，因此無法做到像內建或其他輸入法一樣有強大的前後文語意聯想，屬於「固定式」聯想

- 若無需使用此功能：

  1. 至方案中開啟 liur.schema.yaml

  2. 詢找
     
      name: prediction
      states: [ 關閉預測, 開啟預測 ]
      reset: 1
  3. 將 reset 改為 0
  4. 儲存並重新部署

---

## 十三、無效鍵屏蔽

### 功能說明帶有「屏蔽無效鍵」的方案，可根據當前輸入狀態動態屏蔽無效按鍵。

### 主要特色

1. **屏蔽功能**：根據當前輸入狀態屏蔽無效按鍵
2. **避免誤觸**：減少輸入錯誤，提升輸入準確度

### 屏蔽邏輯

- **輸入進行中**：根據已輸入的編碼，屏蔽不可能的後續按鍵
- **候選顯示**：當有候選字時，保持數字鍵和空格鍵可用

### 適用場景

- **學習階段**：幫助初學者避免輸入無效編碼
- **提升準確度**：減少因誤觸造成的輸入錯誤
- **專注輸入**：讓使用者專注於有效的按鍵選擇

---

## 十四、特殊功能總覽

| 功能     | 快捷鍵/引導鍵                | 說明                                            | 備註                                |
| :------- | :--------------------------- | :---------------------------------------------- | :---------------------------------- |
| 造詞模式 | `;`                          | 臨時造詞功能                                    | 「屏蔽無效鍵」方案不適用            |
| 同音選字 | 左下角`「同音」`鍵或L上滑`'` | 打完字碼後直接按左下角`「同音」`或上滑L帶出 `'` | 需先鍵入蝦米字碼                    |
| 讀音查詢 | `;;`                         | 輸入字碼查讀音                                  | 需開啟元書輸入法「候選Comment功能」 |
| 拼音輸入 | `;'`                         | 漢語拼音輸入中文                                | 下滑QWERT可對應1～5聲調              |
| 快打模式 | `,,sp`                       | 提示可用簡碼                                    | 需開啟元書輸入法「候選Comment功能」 |
| 按鍵說明 | `,,h`                        | 顯示所有快捷鍵說明                              |                                     |

---

## 十五、20250303 版本更新說明

<table>
  <thead>
    <tr>
      <th>功能分類</th>
      <th>功能需求</th>
      <th>提出者</th>
      <th>備註</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>元書內建付費功能</td>
      <td>常用詞</td>
      <td>Leo Huang</td>
      <td>透過元書輸入法內購功能付費給作者(不是我)，即可使用</td>
    </tr>
    <tr>
      <td rowspan="8">方案新增功能</td>
      <td rowspan="6">聯想字</td>
      <td>Xu Zhenxuan</td>
      <td rowspan="6">若無需使用此功能：<br>1. 至方案中開啟 liur.schema.yaml<br>2. 尋找<br>&nbsp;&nbsp;&nbsp;&nbsp;<code>- name: prediction</code><br>&nbsp;&nbsp;&nbsp;&nbsp;<code>states: [ 關閉預測, 開啟預測 ]</code><br>&nbsp;&nbsp;&nbsp;&nbsp;<code>reset: 1</code><br>3. 將 reset 改為 0<br>4. 儲存並重新部署</td>
    </tr>
    <tr>
      <td>Nana Weng</td>
    </tr>
    <tr>
      <td>林思絲</td>
    </tr>
    <tr>
      <td>Fula No</td>
    </tr>
    <tr>
      <td>Sean Wu</td>
    </tr>
    <tr>
      <td>陳嘉興</td>
    </tr>
    <tr>
      <td rowspan="2">強制快打</td>
      <td>Peter Chiang</td>
      <td rowspan="2">若需常駐此功能：<br>1. 至方案中開啟 liur.schema.yaml<br>2. 尋找<br>&nbsp;&nbsp;&nbsp;&nbsp;<code>- name: force_quick_mode</code><br>&nbsp;&nbsp;&nbsp;&nbsp;<code>reset: 0</code><br>&nbsp;&nbsp;&nbsp;&nbsp;<code>states: [ 正常, 強制快打 ]</code><br>3. 將 reset 改為 1<br>4. 儲存並重新部署</td>
    </tr>
    <tr>
      <td>彭世民</td>
    </tr>
    <tr>
      <td rowspan="2">皮膚新增功能</td>
      <td rowspan="2">SE型號無地球</td>
      <td>Dxball Liu</td>
      <td rowspan="2">Shift 鍵下滑，可切換到下一個輸入法</td>
    </tr>
    <tr>
      <td>Kenny Wu</td>
    </tr>
    <tr>
      <td>皮膚功能優化</td>
      <td>長按S鍵應出現＿</td>
      <td>Sean Wu</td>
      <td>長按選選修復</td>
    </tr>
    <tr>
      <td rowspan="5">皮膚設計器</td>
      <td rowspan="2">自定皮膚</td>
      <td>莊嚴</td>
      <td rowspan="5">可透過蝦米輸入法皮膚設計器自定<br><a href="https://ryanwuson.github.io/rime-liur-ios-skin/">https://ryanwuson.github.io/rime-liur-ios-skin/</a><br><br>簡易說明<br><a href="https://github.com/ryanwuson/rime-liur-ios-skin">https://github.com/ryanwuson/rime-liur-ios-skin</a></td>
    </tr>
    <tr>
      <td>陳鈺沛</td>
    </tr>
    <tr>
      <td>Emoji 圖示大小</td>
      <td>陳嘉興</td>
    </tr>
    <tr>
      <td rowspan="2">空白鍵加長</td>
      <td>Leo Huang</td>
    </tr>
    <tr>
      <td>陳嘉興</td>
    </tr>
  </tbody>
</table>

---

**享受流暢的蝦米輸入體驗！** 🦐⌨️

---

*本指南涵蓋了元書輸入法蝦米方案的所有主要功能和使用方法。如有任何問題或建議，歡迎透過專案頁面聯繫我們。*
